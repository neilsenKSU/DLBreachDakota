...
int main(int argc, char* argv[]) {
  ...
  float maxBreachFlow, maxBreachTime, maxBreachWidth, maxBreachDepth, collapseTime, bcWidth, bcDepth, acWidth, acDepth;
  char line[BUF_SIZE];
  int s_len;
  int skip_lines = 5;
  ...
  while((ret_in = read(input2_fd, &buffer, BUF_SIZE)) > 0){
    ...
    while ((linePtr!=NULL)){
      ...
      if (skip_lines <= 0) {
        ...
        memset(line,0,BUF_SIZE);
        /* Roof collapse */
        if (((high==damHeight)||((code==4)&&(width>2*(damHeight-high))))&&(collapsed==0)) {
  ...
  /* Output results */
  memset(line,0,BUF_SIZE);
  sprintf(line,"%9.2f f1\n%9.3f f2\n%9.3f f3\n%9.3f f4\n%9.3f f5\n%9.3f f6\n%9.3f f7\n%9.3f f8\n%9.3f f9\n", maxBreachFlow, maxBreachTime, maxBreachWidth, maxBreachDepth, collapseTime, bcWidth, acWidth, bcDepth, acDepth);
  s_len = strlen(line);
